...
import { addDoc, collection, doc, getDocs, getFirestore, query, setDoc, getDoc } from '@firebase/firestore';
...
const Requests = () => {
    ...
    //     const querySnapshot = await getDocs(query(requestsCollection));
    ...
    //         const newRequestDocRef = doc(requestsCollection);

    //         await setDoc(newRequestDocRef, newItem);

    //         console.log('ID del nuevo documento:', newRequestDocRef.id);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            if (userDoc.exists()) {
                // Crear una colección "requests" en Firestore
                ...
                const requestItem = {
                    ...
                    userId: userId, // ID del usuario que hizo la solicitud
                };
    
                // Guardar la solicitud en la colección "requests"
                await setDoc(newRequestDocRef, requestItem);
    
                // Crear una colección "solicitudes" dentro del documento del usuario
                const userRequestsCollectionRef = collection(userDocRef, 'solicitudes');
                const newRequestDocRefUser = doc(userRequestsCollectionRef);
    
                // Datos de la solicitud para el usuario
                const requestItemUser = {
                    ...newItem,
                    fecha: new Date().toISOString(), // Fecha de la solicitud
                };
    
                // Guardar la solicitud en la colección "solicitudes" del usuario
                await setDoc(newRequestDocRefUser, requestItemUser);
    
                console.log('ID del nuevo documento:', newRequestDocRef.id);
    
                Swal.fire("¡Excelente!", "Has agregado una nueva solicitud", "success");
                reset();
            } else {
                console.error('No se encontró el documento del usuario.');
            }
        } catch (error) {
            Swal.fire("¡Oops!", "Hubo un error al agregar la solicitud", "error");
                ...
                <div className="addProduct__container">
                    ...
                    <form className="w-[80%]" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="flex flex-col w-[100%] mt-8 items-center">
                            <button type="submit" className="button__login fontGreen" onClick={() => navigate('/userRequests')}>
                                Solicitar
                            </button>
                        </div>
                    </form>
